...
export default async function UserProfile() {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center h-screen relative p-4 md:p-8">
      <div className="hidden md:block ml-44 max-h-screen">
        <Image
      ...
      <div className="flex flex-col md:flex-row ml-0 md:ml-8 space-x-0 md:space-x-36 text-white relative z-10">
        <div className="flex flex-col items-start text-center md:text-left">
          <span className="text-6xl md:text-9xl font-bold">
            {user.username}s'
          </span>
          <div className="flex flex-col md:flex-row w-full md:w-96 mt-4 space-y-4 md:space-y-0 md:space-x-4">
            <Link
              className="px-4 py-2 bg-white text-black text-xl font-semibold rounded-md shadow-md hover:bg-black hover:text-white"
              href="/games"
            >
              start new game
            </Link>
            <Link
              className="px-4 py-2 bg-red-950 text-white text-xl font-semibold rounded-md shadow-md hover:bg-red-500"
              href="/games-list"
            >
              join games
            </Link>
        ...
        <span className="text-6xl md:text-9xl font-bold transform md:rotate-90 origin-top-left mt-4 md:mt-0">
          profile
        </span>
      </div>

      <div className="hidden md:block absolute bg-center z-0 transform translate-x-0 translate-y-0 translate-z-0 scale-100 rotate-x-0 rotate-y-0 rotate-10 skew-x-0 skew-y-0 transform-style-preserve-3d">
